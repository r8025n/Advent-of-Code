...
if __name__ == "__main__":
    ...
    for i in range(0, len(galaxy_to_position_map)):
        ...
        for j in range(i + 1, len(galaxy_to_position_map)):
            ...
            expansion_x = len([x for x in empty_rows if x <= max(dest_x, source_x) and x >= min(dest_x, source_x)])
            expansion_y = len([y for y in empty_cols if y <= max(dest_y, source_y) and y >= min(dest_y, source_y)])
